// auth
export const SIGN_UP = "SIGN_UP";
export const SIGN_UP_SUCCESS = "SIGN_UP_SUCCESS";
export const SIGN_UP_FAILURE = "SIGN_UP_FAILURE";

export const SET_ENTREPRENEUR_INFO = "SET_ENTREPRENEUR_INFO";
export const SET_STORE_INFO = "SET_STORE_INFO";

export const SET_BUSINESS_LICENSE = "SET_BUSINESS_LICENSE";
export const SET_PASSWORD = "SET_PASSWORD";

export const NEW_PASSWORD = "NEW_PASSWORD";
export const NEW_PASSWORD_SUCCESS = "NEW_PASSWORD_SUCCESS";
export const NEW_PASSWORD_FAILURE = "NEW_PASSWORD_FAILURE";


export const CHECK_DUPLICATE_ID = "CHECK_DUPLICATE_ID";
export const CHECK_DUPLICATE_ID_SUCCESS = "CHECK_DUPLICATE_ID_SUCCESS";
export const CHECK_DUPLICATE_ID_FAILURE = "CHECK_DUPLICATE_ID_FAILURE";

export const FIND_ID = "FIND_ID";
export const FIND_ID_SUCCESS = "FIND_ID_SUCCESS";
export const FIND_ID_FAILURE = "FIND_ID_FAILURE";

export const FIND_PASSWORD = "FIND_PASSWORD";
export const FIND_PASSWORD_SUCCESS = "FIND_PASSWORD_SUCCESS";
export const FIND_PASSWORD_FAILURE = "FIND_PASSWORD_FAILURE";

//sms
export const SMS_MESSAGE = "SMS_MESSAGE";
export const SMS_MESSAGE_SUCCESS = "SMS_MESSAGE_SUCCESS"
export const SMS_MESSAGE_FAILURE = "SMS_MESSAGE_FAILURE"

export const EXPIRE_SMS_MESSAGE = "EXPIRE_SMS_MESSAGE";
export const VERIFICATION_SMS_MESSAGE = "VERIFICATION_SMS_MESSAGE";

export const MESSAGE_TIME_OUT = "MESSAGE_TIME_OUT";
export const NO_EXIST_MESSAGE = "NO_EXIST_MESSAGE"

//login
export const LOGIN = "LOGIN";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAILURE = "LOGIN_FAILURE";

//autoLogin
export const AUTO_LOGIN = "AUTO_LOGIN";
export const AUTO_LOGIN_SUCCESS = "AUTO_LOGIN_SUCCESS";
export const AUTO_LOGIN_FAILURE = "AUTO_LOGIN_FAILURE";


//entrepreneur
export const LOGOUT = "LOGOUT";
export const DELETE_ENTREPRENEUR = "DELETE_ENTREPRENEUR";
export const DELETE_ENTREPRENEUR_SUCCESS = "DELETE_ENTREPRENEUR_SUCCESS";
export const DELETE_ENTREPRENEUR_FAILURE = "DELETE_ENTREPRENEUR_FAILURE";

export const MODIFY_PASSWORD = "MODIFY_PASSWORD";
export const MODIFY_PASSWORD_SUCCESS = "MODIFY_PASSWORD_SUCCESS";
export const MODIFY_PASSWORD_FAILURE = "MODIFY_PASSWORD_FAILURE";

export const RESET_PASSWORD = "RESET_PASSWORD";
export const RESET_PASSWORD_SUCCESS = "RESET_PASSWORD_SUCCESS";
export const RESET_PASSWORD_FAILURE = "RESET_PASSWORD_FAILURE";



//certificate
export const CERTIFICATE_UPLOAD = "CERTIFICATE_UPLOAD";
export const CERTIFICATE_UPLOAD_SUCCESS = "CERTIFICATE_UPLOAD_SUCCESS";
export const CERTIFICATE_UPLOAD_FAILURE = "CERTIFICATE_UPLOAD_FAILURE";

export const DOWNLOAD_CERTIFICATE = "DOWNLOAD_CERTIFICATE";
export const DOWNLOAD_CERTIFICATE_SUCCESS = "DOWNLOAD_CERTIFICATE_SUCCESS";
export const DOWNLOAD_CERTIFICATE_FAILURE = "DOWNLOAD_CERTIFICATE_FAILURE";


//api
export const API_CHECK_ENTREPRENEUR_STATUS = "API_CHECK_ENTREPRENEUR_STATUS";
export const API_CHECK_ENTREPRENEUR_STATUS_SUCCESS = "API_CHECK_ENTREPRENEUR_STATUS_SUCCESS";
export const API_CHECK_ENTREPRENEUR_STATUS_FAILURE = "API_CHECK_ENTREPRENEUR_STATUS_FAILURE";

export const API_CHECK_ENTREPRENEUR_VAILDATE = "API_CHECK_ENTREPRENEUR_VAILDATE";
export const API_CHECK_ENTREPRENEUR_VAILDATE_SUCCESS = "API_CHECK_ENTREPRENEUR_VAILDATE_SUCCESS";
export const API_CHECK_ENTREPRENEUR_VAILDATE_FAILURE = "API_CHECK_ENTREPRENEUR_VAILDATE_FAILURE";

export const API_REQUEST = "API_REQUEST";

export const NOT_ENTREPRENEUR_NUMBER = "국세청에 등록되지 않은 사업자등록번호입니다.";


//nfc
export const REGISTER_NFC_ID = "REGISTER_NFC_ID";
export const REGISTER_NFC_ID_SUCCESS = "REGISTER_NFC_ID_SUCCESS";
export const REGISTER_NFC_ID_FAILURE = "REGISTER_NFC_ID_FAILURE";

